...
-- We use COUNT(*) instead of COUNT(id) because COUNT(id) does not count null values
...
SELECT country, SUM(bronze) AS num_bronze_medals
FROM countries
	LEFT JOIN summer_games
	ON countries.id = summer_games.country_id
GROUP BY country
ORDER BY num_bronze_medals DESC NULLS LAST
LIMIT 1;
...
SELECT country, cs00.pop_in_millions AS pop_2000, cs00.year, cs06.pop_in_millions AS pop_2006, cs06.year
